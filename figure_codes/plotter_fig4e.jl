...
# Section 4a: Defining the required variables

b = 10; # Number of bins
SIC_fracs = zeros(b, 1); VIR_fracs = zeros(b, 1);

# Section 4b: Estimating the fractions

for i = 1:1:b
    n_SIC = 0; n_VIR = 0;
    for j = 1:1:length(mat_spVL)
        if mat_tAUC[j] > (i-1)/b && mat_tAUC[j] ≤ i/b
            if mat_spVL[j] ≤ log10(400)
                n_SIC = n_SIC + 1;
            else
                n_VIR = n_VIR + 1;
            end
...
# Section 4c: Fit a first-order Hill function to find the threshold ζ

ζ_thres = 0.95;
@. sat(x, p) = p[1] + (1 - p[1])*x/(p[2] + x); # y = a + (1 - a)*x/(b + x)
p0 = [0.5, 0.5]; # Initial guesses for the parameters
fit_4e = curve_fit(sat, 0:0.1:0.9, vec(SIC_fracs), p0);
a1 = coef(fit_4e)[1]; a2 = coef(fit_4e)[2];
@. fit_model(x) = a1 + (1 - a1)*x/(a2 + x); # Final fit model
x_thres = a2*((ζ_thres - a1)/(1 - ζ_thres));

#=
Section 5: Read the model prediction file for macaques
=#

df_dat = DataFrame(XLSX.readtable("fig_4d_data.xlsx","Sheet1";infer_eltypes=true));
exp_tAUC = df_dat.tAUC; exp_spVL = df_dat.spVL;
exp_tAUC_SIC = exp_tAUC[1:12]; exp_tAUC_VIR = exp_tAUC[13:16];

plt_x_exp_tAUC = 0.05:0.1:0.95;
plt_y_exp_frac = zeros(10,1);
...
for j = 1:1:10
    ...
    bin_x = j/10;
    for m = 1:1:16
        if exp_tAUC[m] ≥ bin_x - 0.1 && exp_tAUC[m] < bin_x
            if m < 12
                n_SIC_exp = n_SIC_exp + 1;
            else
                n_VIR_exp = n_VIR_exp + 1;
            end
        else
            continue
        end
    end
    plt_y_exp_frac[j] = n_SIC_exp/(n_SIC_exp + n_VIR_exp);
end
plt_y_exp_frac = vec(plt_y_exp_frac);
...
# Temporary variables required for plotting
x = vcat((1/(2*b)):(1/b):1-(1/(2*b)), (1/(2*b)):(1/b):1-(1/(2*b)));
height = vec(vcat(SIC_fracs, VIR_fracs));
grp = vcat(ones(b, 1), 2*ones(b, 1));
grp = convert(Vector{Int64}, vec(grp));
fit_x = 0:0.01:1;

# Plotting commands
barplot!(ax, x, height, stack = grp, color = grp, colormap = [(:grey, 0.35), (:red, 0.35)],  gap = 0, strokecolor = (:black, 0.35), strokewidth = 0.5);
lines!(fit_x, fit_model.(fit_x), color = :black, linewidth = 1);
vlines!(x_thres, ymax = [ζ_thres], color = :steelblue3, linewidth = 1.5);
scatter!(x_thres, ζ_thres, markersize = 6, color = :steelblue3, strokewidth = 1, strokecolor = :black);
# scatter!(ax, plt_x_exp_tAUC, plt_y_exp_frac, marker = :rect, strokecolor = :black, strokewidth = 1, color = :white, markersize = 6);

xlims!(ax, x_lim);
ylims!(ax, y_lim);

fig
save("fig_4e.svg", fig, px_per_unit = 6)

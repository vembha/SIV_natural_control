...
# Read the data file
df_data = DataFrame(CSV.File("../../figure_data_files/v9_files/master_data_exVivo.csv"));

# Read the parameter file
# Parameter order (Total 9): β_hat_exp, δ, ρ, p, c, f, μ_exp, T0_exp, V0_exp
df_param = DataFrame(XLSX.readtable("../../figure_data_files/v9_files/parameters_model_exVivo.xlsx","Sheet1";infer_eltypes=true))
...
for i = 1:1:3 # Number of rows of panels
    for j = 1:1:6 # Number of macaques per row
        ...
        hidespines!(axl, :t, :r); hidespines!(axr, :t, :l);
        ...
        xlims!(axl, x_lim); xlims!(axr, x_lim);
        ylims!(axl, y_liml); ylims!(axr, y_limr);
        order += 1;
        if order > 18
            break
        end
...
fig
save("fig_S1a.svg", fig, px_per_unit = 6)

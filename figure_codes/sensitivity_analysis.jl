...
function model_1!(du, u, p, t)
    ...
    # ES = 10_000; # Number of SIV-specific effector CD8 T cells in chronic stedy state phase
    # fD = 1 - ((kf*ES + μI)/(λ))*((μT/β_prime) + IS);
    # λE = ES*(μE - αE*(IS/(θE + IS)));
...
function tAUC!(p)
    ...
    u0[1] = p[1]/p[12]
    prob1 = remake(prob; p=p)
    sol1  = solve(prob1, saveat = 0.1, isoutofdomain = (u,p,t)->any(x->x<0,u))
    sol_ψ = (sol1[4,1:281].*sol1[5,1:281])
    return mid_point_int!(time_vec, sol_ψ)/28;
end
...
# Sensitivity of tAUC
# res = gsa(tAUC!, Sobol(), [[0.1*λ, 10*λ], [0.1*β_prime, 10*β_prime], [0.1*fD, 1.0], [0.1*μI, 10*μI], [0.1*μD, 10*μD], [0.1*r, 10*r], [0.1*λE⁺, 10*λE⁺], [0.1*αE, 10*αE], [0.1*θE, 10*θE], [0.1*μE, 10*μE], [0.1ω, 10*ω], [0.1*μT, 10*μT]]; samples = 10_000)

# pT = bar(["λ", "β'", "fD", "μI", "μD", "r", "λE⁺", "αE", "θE", "μE", "ω", "μT"], res.ST[:], title = "Total order indices", legend = false, ylim = (0, 1));
# p1 = bar(["λ", "β'", "fD", "μI", "μD", "r", "λE⁺", "αE", "θE", "μE", "ω", "μT"], res.S1[:], title = "First order indices", legend = false, ylim = (0, 1));
# plot(pT, p1, layout = (2,1))
...
fig
save("fig_S11.svg", fig, px_per_unit = 6)

...
# Parameter order (Total 12): λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, μE, ω_exp T0_exp
df_param = DataFrame(XLSX.readtable("../../figure_data_files/v9_files/parameters_model_1_sans_S_varying_k.xlsx","Sheet1";infer_eltypes=true))
...
for i = 1:1:16
    ...
    prob = ODEProblem(model_1_sans_S_varying_k!,curr_init,tspan,curr_p)
    soln = solve(prob,saveat = 1.0,isoutofdomain=(u,p,t)->any(x->x<0,u))

    # Section 3c: Extract the required values and save them
    sol_V[i,:] .= log10.(2*curr_p[7].*max.(soln[2,:], (1e-6)*ones(length(soln.t))))
...
for i = 1:1:8 # Number of rows of panels
    for j = 1:1:2 # Number of macaques per row
        ...
        for k = 1:1:2 # Number of markers

            # Setting the panel
            if k == 2 # Will have the macaque's name on the top
                if plot_title == "31041" || plot_title == "AV979" || plot_title == "BB598" || plot_title == "BD885"
                    ax = Axis(fig[i, k + 2*(j - 1)], title = plot_title, titlecolor = :red, spinewidth = 1, xtickwidth = 1, ytickwidth = 1, xticks = x_tick, yticks = y_tick, titlefont = "C:/Windows/Fonts/arialbd.ttf");
                else
                    ax = Axis(fig[i, k + 2*(j - 1)], title = plot_title, titlecolor = :black, spinewidth = 1, xtickwidth = 1, ytickwidth = 1, xticks = x_tick, yticks = y_tick, titlefont = "C:/Windows/Fonts/arialbd.ttf");
                end
            else
                if plot_title == "31041" || plot_title == "AV979" || plot_title == "BB598" || plot_title == "BD885"
                    ax = Axis(fig[i, k + 2*(j - 1)], titlecolor = :red, spinewidth = 1, xtickwidth = 1, ytickwidth = 1, xticks = x_tick, yticks = y_tick, titlefont = "C:/Windows/Fonts/arialbd.ttf");
                else
                    ax = Axis(fig[i, k + 2*(j - 1)], titlecolor = :black, spinewidth = 1, xtickwidth = 1, ytickwidth = 1, xticks = x_tick, yticks = y_tick, titlefont = "C:/Windows/Fonts/arialbd.ttf");
                end
            end
            # if k == 1 && i == 8
...
fig
save("fig_S16.svg", fig, px_per_unit = 6)

...
# Parameter order (Total 9): λ, β_hat_exp, λE⁺_exp, dI, r, αE, θE, dE, T0_exp
df_param = DataFrame(XLSX.readtable("../../figure_data_files/v9_files/parameters_model_1_no_D_const_k.xlsx","Sheet1";infer_eltypes=true))
...
for i = 1:1:16
    ...
    init_vec  = [0.0,(10^(-2.76))/(curr_p[5]),0.0];
    curr_T0   = 10^(curr_p[9])
    curr_init = init_vec
    ...
    prob = ODEProblem(model_1_no_D_const_k!,curr_init,tspan,curr_p)
    soln = solve(prob,saveat = 1.0,isoutofdomain=(u,p,t)->any(x->x<0,u))

    # Section 3c: Extract the required values and save them
    sol_V[i,:] .= log10.(2*curr_p[5].*max.(soln[2,:], vec((1e-8)*ones(1,length(time_vec)))))
    sol_I[i,:] .= log10.(soln[2,:])
    sol_S[i,:] .= (soln[3,:]./N).*C
    sol_ψ[i,:] .= (soln[3,:]./N)

    # Section 3d: Estimate S from the predictions for S
    sol_S[i,:] .= S_func!.(sol_S[i,:])
end
...
fig
save("fig_S19.svg", fig, px_per_unit = 6)

...
# Parameter order (Total 16): λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, ξ, ϕ, μE, κ, μQ, ω_exp, T0_exp
df_param = DataFrame(XLSX.readtable("../../figure_data_files/v9_files/parameters_model_4.xlsx","Sheet1";infer_eltypes=true))
...
for i = 1:1:16
    ...
    curr_p    = Vector{Float64}(df_param[i,2:17])
    init_vec  = [0.0,(10^(-2.76))/(curr_p[7]),0.0,0.0,0.0,0.0];
    curr_T0   = 10^(curr_p[16])
    curr_init = init_vec
    ...
    prob = ODEProblem(model_4!,curr_init,tspan,curr_p)
...
fig
save("fig_S4.svg", fig, px_per_unit = 6)

...
function model_1_sans_S_constant_k!(du, u, p, t)
...
function model_1_sans_S_varying_k!(du, u, p, t)
...
function model_1_sans_omega!(du, u, p, t)
...
function model_1_worsening_k!(du, u, p, t)

    T, I, D, E⁺, k⁺ = u; # Variables
    λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, μE, μ, ω, T0_exp = p; # Parameters
    β_prime = 10^(β_prime_exp); μT = (λ/(10^(T0_exp)));
    λE⁺ = 10^(λE⁺_exp);
...
function model_1_increasing_decreasing_k!(du, u, p, t)

    T, I, D, E⁺ = u; # Variables
    λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, μE, μ, ω1_exp, ω2_exp, T0_exp = p; # Parameters
    β_prime = 10^(β_prime_exp); μT = (λ/(10^(T0_exp)));
    λE⁺ = 10^(λE⁺_exp); ω1 = 10^(ω1_exp); ω2 = 10^(ω2_exp);

    du[1] = dT  = λ - β_prime*T*I - μT*T
    du[2] = dI  = (1 - fD)*β_prime*T*I - ((1 - exp(-ω1*t)) - μ*(1 - exp(-ω2*t)))*E⁺*I - μI*I
    du[3] = dD  = fD*β_prime*T*I - μD*D
    du[4] = dE⁺ = λE⁺ + αE*E⁺*I/(θE + I) - μE*E⁺
end

function model_1_no_D_const_k!(du, u, p, t)

    T, I, E⁺ = u; # Variables
    λ, β_prime_exp, λE⁺_exp, μI, r, αE, θE, μE, T0_exp = p; # Parameters
    β_prime = 10^(β_prime_exp); μT = (λ/(10^(T0_exp)));
    λE⁺ = 10^(λE⁺_exp);

    du[1] = dT  = λ - β_prime*T*I - μT*T
    du[2] = dI  = β_prime*T*I - E⁺*I - μI*I
    du[3] = dE⁺ = λE⁺ + αE*E⁺*I/(θE + I) - μE*E⁺
end

function model_1_nonCytolytic!(du, u, p, t)

    T, I, D, E⁺, k⁺ = u; # Variables
    λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, μE, ω_exp, T0_exp, η_str = p; # Parameters
    β_prime = 10^(β_prime_exp); μT = (λ/(10^(T0_exp)));
    λE⁺ = 10^(λE⁺_exp); ω = 10^(ω_exp);

    du[1] = dT  = λ - β_prime*T*I/(1 + η_str*E⁺) - μT*T
    du[2] = dI  = (1 - fD)*β_prime*T*I/(1 + η_str*E⁺) - k⁺*E⁺*I - μI*I
    du[3] = dD  = fD*β_prime*T*I/(1 + η_str*E⁺) - μD*D
    du[4] = dE⁺ = λE⁺ + αE*E⁺*I/(θE + I) - μE*E⁺
    du[5] = dk⁺ = ω*(1 - k⁺)
end

function model_1_lin_k!(du, u, p, t)

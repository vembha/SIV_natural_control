#===========================
PLOTTER FILE FOR 
FIGURE BEST-FIT PLOT FOR 
MODEL WITH INCREASING AND 
DECREASING k
REVISION VERSION #3
===========================#
...
# Parameter order (Total 14): λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, μE, μ, ω1_exp, ω2_exp, T0_exp
df_param = DataFrame(XLSX.readtable("../../figure_data_files/v9_files/parameters_model_1_increasing_decreasing_k.xlsx","Sheet1";infer_eltypes=true))
...
for i = 1:1:16
    ...
    prob = ODEProblem(model_1_increasing_decreasing_k!,curr_init,tspan,curr_p)
    ...
    sol_F[i,:] .= (soln[4,:]./N).*C.*((1 .- exp.(-(10^curr_p[12])*soln.t)) .- curr_p[11].*(1 .- exp.(-(10^curr_p[13])*soln.t)))
    sol_ψ[i,:] .= (soln[4,:]./N)

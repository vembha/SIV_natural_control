...
# Parameter order (Total 13): λ, β_prime_exp, fD, λE⁺_exp, μI, μD, r, αE, θE, μE, μ, ω_exp, T0_exp
df_param = DataFrame(XLSX.readtable("../../figure_data_files/v9_files/parameters_model_1_worsening_k.xlsx","Sheet1";infer_eltypes=true))
...
for i = 1:1:16
    ...
    init_vec  = [0.0,(10^(-2.76))/(curr_p[7]),0.0,0.0,1.0 + curr_p[11]];
    curr_T0   = 10^(curr_p[13])
    curr_init = init_vec
    ...
    prob = ODEProblem(model_1_worsening_k!,curr_init,tspan,curr_p)
